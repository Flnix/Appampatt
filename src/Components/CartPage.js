...
import DeleteIcon from '../Images/trash-fill.svg'; // Add your delete icon path
...
export default function CartPage({ cartItems, handleClearCart, handleRemoveItem }) {
  ...
  const handlePayment = () => {
    ...
    // Ensure Razorpay is available in the window object
    if (window.Razorpay) {
      const razorpayInstance = new window.Razorpay(options);
      razorpayInstance.open();
    } else {
      console.error("Razorpay SDK is not loaded. Make sure you have included the Razorpay script.");
    }
  };
              ...
              <tbody>
                {cartItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <button
                        onClick={() => removeItemFromCart(item)} // Call the function on delete icon click
                        className="btn btn-link p-0" // Styles for button
                      >
                        <img src={DeleteIcon} alt="Delete" style={{ width: '20px', height: '20px' }} /> {/* Use your delete icon */}
                      </button>
                    </td>
                  </tr>
                ))}
              ...
              <Form className="Poppins">
                ...
                <Form.Group className="mb-3" controlId="formGroupEmail">
                  ...
                  <Form.Control
                    ...
                    required
                    onChange={e => setOrderDetails({ ...OrderDetails, EmailAddress: e.target.value })}
                  />
                </Form.Group>
                <Form.Group className="mb-3" controlId="formGroupEmail">
                ...
                <InputGroup className="mb-3">
                  ...
                  <Form.Control
                    ...
                    onChange={e => setOrderDetails({ ...OrderDetails, MobileNumber: e.target.value })}
                  />
                </InputGroup>
                <InputGroup>
                  <InputGroup.Text id="inputGroupPrepend">+91</InputGroup.Text>
                  <Form.Control
                    ...
                    onChange={e => setOrderDetails({ ...OrderDetails, AltMobileNumber: e.target.value })}
                  />
                </InputGroup>
                {cartItems.length > 0 && ( <div className="bottom-overlay d-flex justify-content-between align-items-center p-3">
